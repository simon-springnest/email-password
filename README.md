[README.md](https://github.com/user-attachments/files/32509853/README.md)
# email-password

A small, self-contained HTML tool for generating the password-delivery email sent to customers when a Xneelo-hosted mailbox is created. Fill in three fields and get a ready-to-copy, formatted message — no build step, no dependencies, no server required.
...
1. Open `mailbox-handover.html` in any modern browser (double-click the file, or host it anywhere static files are served).
...
```
mailbox-handover.html   # the entire app — open this file directly
```

## Customizing the template

The email template (greeting, server settings, sign-off, and the closing disclaimer) is defined in the `buildEmailHTML()` function inside `mailbox-handover.html`. Edit the strings there to change the default wording, then reload the page to see the update.
